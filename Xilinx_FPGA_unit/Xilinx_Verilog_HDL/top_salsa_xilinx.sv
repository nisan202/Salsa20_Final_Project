...
module top_salsa_xilinx #(
	parameter ADDR_WIDTH = `ADDR_WIDTH_REGS)
	(
	// Inputs
	input wire			salsa_top_100mhz_clk,
	input wire			salsa_top_rst_n,
	input wire			rs232_rx_top,  // rx top
	
	// Outputs 
	output reg			rs_232_tx_top, // tx top
	output reg			hash_done_sticky_top,	// hash_done_sticky go to LD0 on Basys3
	output reg			xor_valid_sticky_top,	// xor_valid_sticky go to LD1 on Basys3
	
	output reg			salsa_top_pll_locked,	// go to LD2 on Basys3
	
	output reg [3:0]	anode_set_top,
	output wire 		dp_set_top,
	output reg [6:0]	seven_seg_top
	
// TODO:
// add input clear_from_Board and send to res then add this signal to clear sticky
);

// Internal Signals
logic					salsa_top_80mhz_pll_clk;
...
// insance of clk_wiz_0.xci Xilinx IP PLL

clk_wiz_0	salsa_top_xilinix_ip_pll(
// Inputs
	.clk_in1		(salsa_top_100mhz_clk),		// 100MHz from Basys3 to pll
	.resetn			(salsa_top_rst_n),			// active low reset 
// Outputs
	.clk_out1		(salsa_top_80mhz_pll_clk),	// 80MHz from pll to salsa sources
	.locked			(salsa_top_pll_locked)		// go to LD2 on Basys3
);
// -----------------------------------------

/*
// instance of salsa_hashing_wrap

salsa_hashing_wrap u_salsa_hashing_wrap(
// Inputs
...
// Inputs
	.clk						(salsa_top_80mhz_pll_clk),	// general regs IF
...
// -----------------------------------------
// instance of hex_to_SSD

hex_to_SSD u_hex_to_SSD(
// Inputs
	.hex_number		(state2ssd_top),
	.anode_number	(2'h3),
	.dp				(1'b1),
// Outputs
	.anode_set		(anode_set_top),
	.dp_set			(dp_set_top),
